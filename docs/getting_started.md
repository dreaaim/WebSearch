...
```bash
pip install -i https://test.pypi.org/simple/ trusted-web-search
```

### 1.2 安装可选依赖

```bash
pip install -i https://test.pypi.org/simple/ trusted-web-search[js]
```
...
```python
import asyncio
from web_search.core.orchestrator import SearchOrchestratorV3
...
async def main():
    ...
    llm_client, thinking_llm_client = create_llm_clients(config.get("llm", {}))
    embedding_client = create_embedding_client(config.get("embedding", {}))
    reranker_client = create_reranker_client(config.get("reranker", {}))
    ...
    result = await orchestrator.search_with_trust_v3("人工智能最新进展")
...
默认配置目录为 `configs`，需包含以下配置文件：

- `llm.yaml` - LLM 配置
- `embedding.yaml` - Embedding 配置
- `reranker.yaml` - Reranker 配置

### 3.2 LLM 配置示例 (llm.yaml)
...
### 3.3 Embedding 配置示例 (embedding.yaml)
...
### 3.4 Reranker 配置示例 (reranker.yaml)
...
`search_with_trust_v3` 返回 `TrustedSearchResultV3` 对象，包含以下字段：

| 字段 | 类型 | 说明 |
|------|------|------|
| query | str | 原始查询 |
| trusted_facts | List[TrustedFact] | 可信事实清单 |
| all_facts | List[ExtractedFact] | 所有提取的事实 |
| buckets | List[FactBucket] | 事实桶列表 |
| collisions | List[CollisionResult] | 碰撞检测结果 |
| metadata | dict | 处理统计元数据 |

### 4.1 metadata 常用字段

```python
result.metadata["total_search_results"]  # 搜索结果总数
result.metadata["unique_results"]         # 去重后结果
result.metadata["filtered_results"]         # 过滤后结果
result.metadata["successful_fetches"]       # 成功抓取数
result.metadata["total_facts_extracted"]   # 提取事实数
result.metadata["total_buckets"]           # 事实桶数
result.metadata["total_collisions"]        # 碰撞检测数
result.metadata["trusted_facts_count"]     # 可信事实数
result.metadata["duration_ms"]              # 处理耗时(ms)
```
...
- `undetected-chromedriver>=3.5.0` - JavaScript 渲染支持
- `playwright>=1.40.0` - Playwright 渲染支持
